...
setInterval(function () {
    ...
    if (mode == 0) {
        ...
        if (
            ...
            var move_to_x = character.real_x + (target.real_x - character.real_x);
            var move_to_y = character.real_y + (target.real_y - character.real_y);
            draw_line(character.x, character.y , move_to_x, move_to_y, 1, 0x00ff00);
            lines_drawn++;
            game_log("Moving towards " + target.name);
            moving = true;
            move(
                move_to_x,
                move_to_y
        ...
        if (
            target &&
            parent.distance(character, target) < (character.range * .5)
        ) {
            game_log("Moving away " + target.name)
            move(
